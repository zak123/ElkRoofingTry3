...
function initTestimonialSlider() {
    ...
    testimonialSlider.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(diff) > 50) { // Simple threshold
            if (diff > 0) {
...
function initContactForm() {
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        if (isValid) {
            ...
            successMessage.style.cssText = 'background-color: #d4edda; color: #155724; padding: 15px; border-radius: 5px; margin-bottom: 20px; text-align: center;';
            successMessage.innerHTML = '<strong>Thank you!</strong> Your message has been sent. Our team will contact you shortly.';
...
function initAnimations() {
    ...
    function checkScroll() {
        ...
        } else if (aboutImgRect.bottom <= endPoint || aboutImgRect.top >= startPoint) {
    ...
    function applyZoomEffectToServices() {
        serviceCards.forEach(card => {
            ...
            if (cardRect.top < windowHeight && cardRect.bottom > 0) {
                ...
                const zoomAmount = 1.1 + (Math.min(Math.max(viewportPosition, 0), 1) * 0.05);
